...
function defaultSalary(){return{name:'',gross:0,inssBase:0,inssManual:0,useINSS:true,showFGTS:true,useTransport:false,useFood:false,foodMode:'value',foodValue:0,useHealth:false,healthMode:'value',healthValue:0}}
...
function calcOneSalary(s){const gross=toNumber(s.gross);const inssBase=toNumber(s.inssBase)>0?toNumber(s.inssBase):gross;const manualInss=toNumber(s.inssManual);const inss=s.useINSS?(manualInss>0?manualInss:calculateINSS(inssBase)):0;const fgts=s.showFGTS?gross*0.08:0;const transport=s.useTransport?gross*0.06:0;const food=benefitDeduct(s.useFood,s.foodMode,toNumber(s.foodValue),gross);const health=benefitDeduct(s.useHealth,s.healthMode,toNumber(s.healthValue),gross);const totalDeductions=inss+transport+food+health;const net=Math.max(0,gross-totalDeductions);return{name:s.name||'',gross,inss,inssBase,inssManual:manualInss,fgts,transport,food,health,totalDeductions,net}}
...
function fillSalaryFields(prefix, salary){
  ...
  if($(prefix+'InssBase'))$(prefix+'InssBase').value=salary.inssBase||'';
  if($(prefix+'InssManual'))$(prefix+'InssManual').value=salary.inssManual||'';
}
function openSettings(){const s=normalizeSalary(state.salary);state.salary=s;$('grossSalary').value=s.gross||'';$('inssBase1').value=s.inssBase||'';$('inssManual1').value=s.inssManual||'';$('salaryName1').value=s.name||'';$('manualBalance').value=s.manualExtra||'';$('useSecondSalary').checked=!!s.useSecondSalary;$('useINSS').checked=!!s.useINSS;$('showFGTS').checked=!!s.showFGTS;$('useTransport').checked=!!s.useTransport;$('useFood').checked=!!s.useFood;$('foodMode').value=s.foodMode||'value';$('foodValue').value=s.foodValue||'';$('useHealth').checked=!!s.useHealth;$('healthMode').value=s.healthMode||'value';$('healthValue').value=s.healthValue||'';const s2=s.second||defaultSalary();$('grossSalary2').value=s2.gross||'';$('inssBase2').value=s2.inssBase||'';$('inssManual2').value=s2.inssManual||'';$('salaryName2').value=s2.name||'';$('useINSS2').checked=!!s2.useINSS;$('showFGTS2').checked=!!s2.showFGTS;$('useTransport2').checked=!!s2.useTransport;$('useFood2').checked=!!s2.useFood;$('foodMode2').value=s2.foodMode||'value';$('foodValue2').value=s2.foodValue||'';$('useHealth2').checked=!!s2.useHealth;$('healthMode2').value=s2.healthMode||'value';$('healthValue2').value=s2.healthValue||'';updateSalaryPreview();$('settingsDialog').showModal()}
function readSalaryPerson(suffix=''){const n=suffix||'1';return{name:$('salaryName'+n).value.trim(),gross:toNumber($('grossSalary'+suffix).value),inssBase:toNumber($('inssBase'+n).value),inssManual:toNumber($('inssManual'+n).value),useINSS:$('useINSS'+suffix).checked,showFGTS:$('showFGTS'+suffix).checked,useTransport:$('useTransport'+suffix).checked,useFood:$('useFood'+suffix).checked,foodMode:$('foodMode'+suffix).value,foodValue:toNumber($('foodValue'+suffix).value),useHealth:$('useHealth'+suffix).checked,healthMode:$('healthMode'+suffix).value,healthValue:toNumber($('healthValue'+suffix).value)}}
...
function salaryBlock(title,c){const baseInfo=c.inssBase&&Math.abs(c.inssBase-c.gross)>0.009?`<div class="line"><span>Base do INSS</span><b>${money(c.inssBase)}</b></div>`:'';const manualInfo=c.inssManual>0?`<div class="line"><span>INSS manual do holerite</span><b>-${money(c.inssManual)}</b></div>`:'';return`<div class="salary-person-preview"><strong>${escapeHtml(title)}</strong><div class="line"><span>Salário bruto</span><b>${money(c.gross)}</b></div>${baseInfo}<div class="line"><span>INSS</span><b>-${money(c.inss)}</b></div>${manualInfo}<div class="line"><span>FGTS estimado, não descontado</span><b>${money(c.fgts)}</b></div><div class="line"><span>Vale-transporte</span><b>-${money(c.transport)}</b></div><div class="line"><span>Vale alimentação</span><b>-${money(c.food)}</b></div><div class="line"><span>Plano de saúde</span><b>-${money(c.health)}</b></div><div class="line"><span>Líquido da pessoa</span><b>${money(c.net)}</b></div></div>`}
...
['grossSalary','inssBase1','inssManual1','salaryName1','manualBalance','useSecondSalary','useINSS','showFGTS','useTransport','useFood','foodMode','foodValue','useHealth','healthMode','healthValue','grossSalary2','inssBase2','inssManual2','salaryName2','useINSS2','showFGTS2','useTransport2','useFood2','foodMode2','foodValue2','useHealth2','healthMode2','healthValue2'].forEach(id=>$(id).addEventListener('input',updateSalaryPreview));
