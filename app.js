...
function renderPremiumV6(){const m=buildPremiumMetrics();if($('financeScore'))$('financeScore').textContent=m.score;if($('scoreText'))$('scoreText').textContent=m.score>=80?'Excelente controle financeiro.':m.score>=60?'Bom, mas ainda há pontos para otimizar.':'Atenção: revise dívidas, saldo e pagamentos.';document.body.classList.toggle('values-hidden',!!state.security?.hideValues);if($('toggleValuesBtn'))$('toggleValuesBtn').textContent=state.security?.hideValues?'Mostrar valores':'Ocultar valores';renderGoals();renderAI();renderCharts();renderHistory();renderAchievements()}
